...
public class EmpOperationsController {
	...
	/*
	 * @DeleteMapping("/delete/{empId}") public ResponseEntity<?>
	 * deleteEmploy(@PathVariable int empId){
	 * 
	 * try { Employ employ = employRepository.findById(empId).get();
	 * employRepository.delete(employ); return new
	 * ResponseEntity<>("Deleted...",HttpStatus.OK); } catch(Exception e) { return
	 * new ResponseEntity<>("No Record Found..",HttpStatus.OK); }
	 * 
	 * }
	 * 
	 * @PutMapping("/update") public ResponseEntity<?> update( @RequestBody Employ
	 * newEmploy) { try { Employ employ =
	 * employRepository.findById(newEmploy.getEmployId()).get();
	 ...
	 * employRepository.save(employ); return new
	 * ResponseEntity<>("updated.....",HttpStatus.OK); } catch(Exception e) { return
	 * new ResponseEntity<>("No Record Found..",HttpStatus.OK); } }
	 * 
